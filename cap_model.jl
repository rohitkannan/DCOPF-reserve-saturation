# CONSERVATIVE FORMULATION THAT USES THE AFFINE POLICY ALONG WITH A TIGHT CONSTRAINT ON THE PROBABILITY OF GENERATOR BOUNDS VIOLATION
...
#*============ GENERATOR BOUNDS VIOLATION TOLERANCE ==========

# upper bound on probability of violation of each individual generator's bounds
const genBoundEps_val = logspace(-3,-2,2)
# use tighter chance constraints for the 118 bus case because it has more lines
if(model == "118bus")
	genBoundEps_val = logspace(-5,-4,2)
end

# number of generator bound violation probability iterations
const numGenBoundEpsIter = size(genBoundEps_val,1)

# compute the quantiles corresponding to the generator violation probability allowances
const quantileEps_val = Inf*ones(Float64,numGenBoundEpsIter)
for iter_gen = 1:numGenBoundEpsIter
	quantileEps_val[iter_gen] = norminvcdf(1.0-genBoundEps_val[iter_gen])
end
...
# solve the CAP SAA model (SOCP) with penalties on generator bounds violation beyond a pre-determined fraction
function solveCAPModel(quantileEps::Float64,gamma_line::Float64,
							delta_line::Float64,omega_d::Array{Float64},
							omega_w::Array{Float64},useGurobi::Bool=true)
	...
	@variable(mod, linePenalty[1:numLines,1:numScenarios] >= 0)

	
	# objective function
	@objective(mod, Min, sum(genCost[i]*p0[i] + posResCost[i]*rp[i] + negResCost[i]*rm[i] for i=1:numGen) + (1.0/numScenarios)*sum(gamma_res*posResCost[i]*posResViol[i,scen] + gamma_res*negResCost[i]*negResViol[i,scen] for scen=1:numScenarios for i = 1:numGen) + (1.0/numScenarios)*sum(linePenalty[k,scen] for k = 1:numLines for scen = 1:numScenarios))
	...
	# constraints defining p: affine recourse policy
	@constraint(mod, [i=1:numGen,scen=1:numScenarios], p[i,scen] == p0[i] + alpha[i]*sumDemandDeviations[scen])
	
	# chance constraints on generator limits satisfaction
	# first, chance constraints on regular generators, where the random variables correspond to sum of demand deviations
	@constraint(mod, cc_reg1[i=1:numRegGen], p0[i] + quantileEps*alpha[i]*sumDemandDeviationStdev <= pmax[i])
	@constraint(mod, cc_reg2[i=1:numRegGen], p0[i] - quantileEps*alpha[i]*sumDemandDeviationStdev >= pmin[i])
	
	# next, chance constraints on wind generators, where the random variables are the sum of demand deviations and the wind availability
	@constraint(mod, cc_wind1[i=1:numWindGen], p0[i+numRegGen] + quantileEps*norm([alpha[i+numRegGen]*sumDemandDeviationStdev,windPredStdev[i]]) <= windPredMean[i])
	@constraint(mod, cc_wind2[i=1:numWindGen], p0[i+numRegGen] - quantileEps*alpha[i+numRegGen]*sumDemandDeviationStdev >= pmin[i+numRegGen])
	...
	# write SOCP reformulation of line penalty term 
	@constraint(mod, [k=1:numLines,scen=1:numScenarios], norm([2*sqrt(gamma_line)*lineViol[k,scen], 1.0-linePenalty[k,scen]]) <= 1.0 + linePenalty[k,scen])
	...
	if(solve_status != :Optimal && solve_status != :Suboptimal)
		if(useGurobi)
			println("CAP model not solved to (sub)optimality using Gurobi!!!")
		else
			println("CAP model not solved to (sub)optimality using Ipopt!!!")
		end
...
const baseDirName = "C:/Users/rkannan/Desktop/DCOPF-reserve-saturation/experiments/" * model * "/" * "cap_" * caseID
mkpath(baseDirName)
...
if(storeResults)
	...
	open(details_file, "w") do f
		...
		write(f,"======================\n")
		write(f,"Individual generator bounds violation probability upper bound: $genBoundEps_val \n\n")
	...
	# write the line penalty coefficients to a file
	gen_eps_file = subDirName * "gen_eps.txt"
	open(gen_eps_file,"w") do f
		for iter_gen = 1:numGenBoundEpsIter
			write(f,"$(genBoundEps_val[iter_gen])\n")
		end
	end
	...
	if(rep >= startingRep)
		...
		for iter_line = 1:numGammaLineIter
			...
			for iter_gen = 1:numGenBoundEpsIter
				...
				println("\n*********************")
				println("GEN VIOL ITER ",iter_gen,"/",numGenBoundEpsIter," with violation bound ",genBoundEps_val[iter_gen],"\n")
			
				tic()

				p0_soln, rp_soln, rm_soln, alpha_soln, solve_status = solveCAPModel(quantileEps_val[iter_gen],gamma_line_val[iter_line],delta_line,omega_d,omega_w)
				
				# if Gurobi did not solve the model as expected, try IPOPT
				if(solve_status != :Optimal && solve_status != :Suboptimal)
					println("--- Gurobi did not converge! Going to try IPOPT ---")
					p0_soln, rp_soln, rm_soln, alpha_soln, solve_status = solveCAPModel(quantileEps_val[iter_gen],gamma_line_val[iter_line],delta_line,omega_d,omega_w,false)
				end
				
				solnTime::Float64 = toq()
				
				if(solve_status != :Optimal && solve_status != :Suboptimal)
					println("*****SKIPPING ITERATION BECAUSE BOTH IPOPT DID NOT CONVERGE!*****")
				else	
					# NOTE: the projection changes the solution for alpha quite a bit, but we need to project to ensure that the recourse solution always exists
					p0_soln, rp_soln, rm_soln, alpha_soln, ~ = projectOntoFeasibleSet(p0_soln, rp_soln, rm_soln, alpha_soln)
					
			
					if(printIterSoln)
						println("CAP solution. Regular generator variables first \n")
